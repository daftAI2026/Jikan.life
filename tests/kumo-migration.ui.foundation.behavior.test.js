...
test("ColorPicker uses Kumo Tooltip Portal for trigger HEX hover instead of inline CSS tooltip", () => {
  ...
  assert.match(source, /className="grid min-w-0 grid-cols-\[36px_minmax\(0,2fr\)_minmax\(0,2fr\)\] items-center gap-2"/)
  assert.match(source, /className="h-9 min-w-0 w-full rounded-lg text-sm font-medium uppercase"/)
  assert.match(source, /className="h-9 min-w-0 w-full rounded-lg text-center font-mono text-sm uppercase"/)
  assert.match(source, /text-sm font-medium tabular-nums/)
  assert.doesNotMatch(source, /text-xs font-mono text-kumo-default/)
