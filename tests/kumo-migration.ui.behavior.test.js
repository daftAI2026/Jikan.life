...
test("Registry settings colors keep lg picker layout and hide presets in mid year/goal", () => {
  ...
  assert.match(fieldShellSource, /mb-1\.5 inline-flex items-center gap-1 text-xs text-kumo-subtle/)
  assert.match(fieldShellSource, /function CardField\(\{\s*label,\s*hint,\s*tooltip,\s*children,\s*className = "",\s*labelClassName = ""\s*\}\)/)
  assert.match(fieldShellSource, /import \{ Tooltip,\s*TooltipProvider \} from "@\/components\/ui\/tooltip"/)
  assert.match(fieldShellSource, /import \{ Info \} from "@phosphor-icons\/react"/)
  assert.match(fieldShellSource, /tooltip && \(/)
  assert.match(fieldShellSource, /<Tooltip content=\{tooltip\} asChild>/)
  ...
  assert.match(source, /<CardField label=\{t\("config\.accent"\)\} tooltip=\{t\("config\.accentTooltip"\)\}[\s\S]*?>[\s\S]*?<ColorPicker[\s\S]*?showValue=\{false\}/)
...
test("HomeGrid provides split workspace layout", () => {
  ...
  assert.match(source, /effectiveLayoutTier,\s*useSegmentedWorkspaceLayout,\s*workspaceHeight,/)
  ...
  assert.match(source, /\? "h-full grid-rows-\[minmax\(0,1fr\)_auto\] overflow-y-hidden"/)
  assert.match(source, /: "md:h-\[calc\(100vh-var\(--registry-topbar-height\)\)\] md:grid-rows-\[minmax\(0,1fr\)_auto\] md:overflow-hidden"/)
  ...
  assert.doesNotMatch(source, /data-registry-pane="settings"[\s\S]*?className=\{useSegmentedWorkspaceLayout \? "min-h-0"/)
...
test("HomePreviewPane renders year and goal as inline SVG without the canvas renderer", () => {
  const source = readSource("src/pages/registry/sections/workspace/HomePreviewPane.jsx")
  ...
  assertNamedImports(source, "./YearPreviewSvg", ["YearPreviewSvg"])
  assertNamedImports(source, "./GoalPreviewSvg", ["GoalPreviewSvg"])
  assert.match(source, /const showWidgets = config\.selectedType !== "goal"/)
  assert.match(source, /const previewContent = config\.selectedType === "year"/)
  assert.match(source, /YearPreviewSvg/)
  assert.match(source, /GoalPreviewSvg/)
  assert.match(source, /: config\.selectedType === "goal"/)
  assert.doesNotMatch(source, /@\/lib\/renderer/)
  assert.doesNotMatch(source, /drawYearProgress/)
  assert.doesNotMatch(source, /drawLifeCalendar/)
  assert.doesNotMatch(source, /drawGoalCountdown/)
  assert.doesNotMatch(source, /canvasRef/)
  assert.doesNotMatch(source, /useCallback/)
  assert.doesNotMatch(source, /useEffect/)
  assert.doesNotMatch(source, /useRef/)
  assert.doesNotMatch(source, /previewScale/)
  assert.doesNotMatch(source, /devicePixelRatio/)
  assert.doesNotMatch(source, /ctx\.setTransform/)
  assert.doesNotMatch(source, /ctx\.scale/)
  assert.doesNotMatch(source, /<canvas/)

  assert.match(yearSource, /aria-label="Wallpaper live preview"/)
  assert.match(yearSource, /viewBox=\{`0 0 \$\{baseWidth\} \$\{baseHeight\}`\}/)
  assert.match(yearSource, /width="100%"/)
  assert.match(yearSource, /height="100%"/)
  assert.match(yearSource, /preserveAspectRatio="xMidYMid slice"/)
  assert.doesNotMatch(yearSource, /shrink-0/)
  ...
  assert.match(goalSource, /aria-label="Wallpaper live preview"/)
  assert.match(goalSource, /viewBox=\{`0 0 \$\{baseWidth\} \$\{baseHeight\}`\}/)
  assert.match(goalSource, /width="100%"/)
  assert.match(goalSource, /height="100%"/)
  assert.match(goalSource, /preserveAspectRatio="xMidYMid slice"/)
  assert.doesNotMatch(goalSource, /shrink-0/)
  assert.match(goalSource, /getGoalRingGeometry\(layout\.ring\.progress\)/)
  assert.match(goalSource, /strokeDasharray=\{circumference\}/)
  assert.match(goalSource, /strokeDashoffset=\{circumference \* ringGeometry\.strokeDashoffsetRatio\}/)
  assert.match(goalSource, /rotate\(-90/)
  assert.match(goalSource, /textAnchor="middle"/)
  assert.match(goalSource, /dominantBaseline="middle"/)
  assert.match(goalSource, /resolveTextFontFamily\(config\.wallpaperLang,\s*layout\.goalName\)/)
  assert.match(goalSource, /getWallpaperFontFamily\(config\.wallpaperLang\)/)
})
...
test("mobile preview sizing derives target height from full first-card budget instead of legacy guesswork", () => {
  ...
  assert.match(source, /const MOBILE_PREVIEW_MAX_TARGET_HEIGHT = DEFAULT_LOCK_SCREEN_TARGET_HEIGHT/)
  ...
  assert.match(source, /function resolvePreviewTargetHeight\(\{ effectiveLayoutTier, useSegmentedWorkspaceLayout, workspaceHeight \}\)/)
  assert.match(source, /if \(!useSegmentedWorkspaceLayout\) return DEFAULT_LOCK_SCREEN_TARGET_HEIGHT/)
  assert.match(source, /if \(!\["mobile", "md"\]\.includes\(effectiveLayoutTier\)\) return DEFAULT_LOCK_SCREEN_TARGET_HEIGHT/)
