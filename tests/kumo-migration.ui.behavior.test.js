...
test("HomeSettingsPane splits md drawer-open bottom-tabs layout from grid layout", () => {
  ...
  assert.match(source, /function resolveMdBottomTabsDistributedWidths\(/)
  ...
  assert.match(source, /useLayoutEffect\(\(\) => \{[\s\S]*?const naturalWidths = measureTriggerRefs\.current/)
  ...
  assert.match(source, /const nextWidths = resolveMdBottomTabsDistributedWidths\(\{/)
  assert.match(source, /naturalWidths: naturalTabWidthsRef\.current/)
  assert.match(source, /containerWidth: tabsListElement\.getBoundingClientRect\(\)\.width/)
  assert.doesNotMatch(source, /const scheduleMeasurement = \(\) => \{[\s\S]*?requestAnimationFrame\(measureTabWidths\)/)
  ...
  assert.match(source, /indicatorClassName=\{isResizingTabs \? "!transition-none" : undefined\}/)
  ...
  assert.match(source, /w-\[var\(--md-tab-w-0\)\]/)
  assert.match(source, /\[flex:0_0_var\(--md-tab-w-0\)\]/)
...
test("HomeSettingsPane md bottom-tabs initialize widths synchronously and only animate indicator outside live resize", () => {
  const source = readSource("src/pages/registry/sections/workspace/HomeSettingsPane.jsx")

  assert.match(source, /const resizeSettledTimeoutRef = useRef\(null\)/)
  assert.match(source, /setDistributedTabWidths\(resolveMdBottomTabsDistributedWidths\(/)
  assert.match(source, /setIsResizingTabs\(true\)/)
  assert.match(source, /window\.clearTimeout\(resizeSettledTimeoutRef\.current\)/)
  assert.match(source, /resizeSettledTimeoutRef\.current = window\.setTimeout\(\(\) => \{/)
  assert.match(source, /setIsResizingTabs\(false\)/)
  assert.match(source, /indicatorClassName=\{isResizingTabs \? "!transition-none" : undefined\}/)
})
