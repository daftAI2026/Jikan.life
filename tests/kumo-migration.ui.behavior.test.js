...
test("HomeSettingsPane splits md drawer-open bottom-tabs layout from grid layout", () => {
  ...
  assert.match(source, /const availableWidth = tabsListElement\?\.getBoundingClientRect\(\)\.width \?\? containerElement\.getBoundingClientRect\(\)\.width/)
  assert.match(source, /const nextWidths = resolveMdBottomTabWidths\(\{ naturalWidths, containerWidth: availableWidth \}\)/)
