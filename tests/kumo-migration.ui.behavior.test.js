...
test("HomeSettingsPane splits md drawer-open bottom-tabs layout from grid layout", () => {
  ...
  assertNamedImports(source, "@/components/ui/kumo", ["SkeletonLine", "Tabs"])
  assert.match(source, /import \{ useRef, useState \} from "react"/)
  assert.match(source, /import \{ useMdBottomTabsMetrics \} from "\.\/use-md-bottom-tabs-metrics"/)
  assert.match(source, /useMdBottomTabsLayout = false/)
  assert.match(source, /const \[activeTabId, setActiveTabId\] = useState\(null\)/)
  assert.match(source, /const MD_BOTTOM_TABS_EMPTY_STATE_CARD_IDS = \["location", "wallpaper-lang", "goal-fields", "colors", "device", "url"\]/)
  assert.match(source, /function resolveMdBottomTabsCardOrder\(selectedType\)/)
  assert.match(source, /function resolveMdBottomTabsRevealedCardIds\(selectedType, cardOrder, unlockedCount\)/)
  assert.match(source, /const bottomTabsCardOrder = resolveMdBottomTabsCardOrder\(config\.selectedType\)/)
  assert.match(source, /const revealedTabs = resolveMdBottomTabsRevealedCardIds\(config\.selectedType, cardOrder, unlockedCount\)/)
  assert.match(source, /const currentActiveTab = bottomTabsCardOrder\.includes\(activeTabId\) \? activeTabId : bottomTabsCardOrder\[0\]/)
  assert.match(source, /if \(useMdBottomTabsLayout\) return <HomeSettingsPaneBottomTabsLayout/)
  assert.match(source, /function HomeSettingsPaneBottomTabsLayout\(/)
  assert.match(source, /const measureLabels = bottomTabsCardOrder\.map\(\(cardId\) => resolveMdTabLabel\(cardId, t\)\)/)
  assert.match(source, /const \{ distributedTabWidths, indicatorClassName \} = useMdBottomTabsMetrics\(\{/)
  assert.match(source, /tabsContainerRef,/)
  assert.match(source, /measureTriggerRefs,/)
  assert.match(source, /measureLabels,/)
  assert.match(source, /const bottomTabsWidthVars = resolveMdBottomTabsWidthVars\(distributedTabWidths\)/)
  assert.match(source, /function resolveMdBottomTabsWidthVars\(widths\)/)
  assert.match(source, /function resolveMdBottomTabTriggerClassName\(index\)/)
  assert.match(source, /function resolveMdBottomTabLabelContent\(cardId, isLabelRevealed, t\)/)
  assert.match(source, /function resolveMdBottomTabsActiveTitle\(/)
  assert.match(source, /<Tabs/)
  assert.match(source, /variant="segmented"/)
  assert.match(source, /listClassName="w-full"/)
  assert.match(source, /indicatorClassName=\{indicatorClassName\}/)
  assert.match(source, /tabs=\{bottomTabsCardOrder\.map/)
  assert.match(source, /label: resolveMdBottomTabLabelContent\(cardId, revealedTabs\.includes\(cardId\), t\)/)
  assert.match(source, /data-home-md-bottom-tabs/)
  assert.match(source, /style=\{bottomTabsWidthVars\}/)
  assert.match(source, /const MD_BOTTOM_TABS_SLOT_COUNT = 6/)
  assert.match(source, /const MD_BOTTOM_TAB_WIDTH_VAR_PREFIX = "--md-tab-w-"/)
  assert.match(source, /function resolveMdBottomTabsWidthVarName\(index\)/)
  assert.match(source, /Array\.from\(\{ length: MD_BOTTOM_TABS_SLOT_COUNT \}, \(_, index\) => \{/)
  assert.match(source, /w-\[var\(\$\{widthVarName\}\)\]/)
  assert.match(source, /\[flex:0_0_var\(\$\{widthVarName\}\)\]/)
  assert.match(source, /className: resolveMdBottomTabTriggerClassName\(index\)/)
  assert.match(source, /value=\{currentActiveTab/)
  assert.match(source, /onValueChange=\{setActiveTabId\}/)
  assert.match(source, /style=\{\{ height: "var\(--registry-topbar-height\)" \}\}/)
  assert.match(source, /w-full/)
  assert.match(source, /flex-1 min-h-0/)
  assert.doesNotMatch(source, /listClassName="[^"]*bg-transparent/)
  assert.doesNotMatch(source, /listClassName="[^"]*h-full/)
  assert.doesNotMatch(source, /listClassName="[^"]*py-1/)
  assert.doesNotMatch(source, /className: "min-w-0 basis-0 flex-1/)
  assert.doesNotMatch(source, /style=\{distributedTabWidths\[index\][\s\S]*width:/)
  assert.doesNotMatch(source, /const visibleTabs =/)
  assert.doesNotMatch(source, /useEffect\([\s\S]*setActiveTabId/)
  assert.doesNotMatch(source, /useLayoutEffect\(/)
  assert.doesNotMatch(source, /resolveMdBottomTabWidths/)
  assert.doesNotMatch(source, /ResizeObserver/)
  assert.doesNotMatch(source, /requestAnimationFrame/)
  assert.doesNotMatch(source, /document\.fonts/)
  assert.doesNotMatch(source, /naturalTabWidthsRef/)
  assert.doesNotMatch(source, /lastTablistWidthRef/)
  assert.doesNotMatch(source, /resizeSettledTimeoutRef/)
  assert.doesNotMatch(source, /isResizingTabs/)
...
test("HomeSettingsPane md bottom-tabs keep shell mounted and reveal title-label text independently from card body", () => {
  ...
  assert.match(source, /const revealedTabs = resolveMdBottomTabsRevealedCardIds\(config\.selectedType, cardOrder, unlockedCount\)/)
  assert.match(source, /const currentActiveTab = bottomTabsCardOrder\.includes\(activeTabId\) \? activeTabId : bottomTabsCardOrder\[0\]/)
  assert.match(source, /const isActiveTabRevealed = revealedTabs\.includes\(currentActiveTab\)/)
  assert.match(source, /const activeTitleState = resolveMdBottomTabsActiveTitle\(/)
  assert.match(source, /title=\{activeTitleState\.title\}/)
  assert.match(source, /titleTooltip=\{activeTitleState\.titleTooltip\}/)
  assert.match(source, /isIndexActive=\{activeTitleState\.isIndexActive\}/)
  assert.match(source, /isActiveTabRevealed && activeCardDefinition[\s\S]*?\? activeCardDefinition\.card\.render\(cardViewModel\)\s*:\s*<SettingsCardSkeleton onRequestRevealAll=\{cardViewModel\.config\.selectedType \? onRequestRevealAll : undefined\} \/>/)
  assert.match(source, /if \(!currentActiveTab \|\| !isActiveTabRevealed \|\| !activeCardDefinition\)/)
  assert.match(source, /title: <SettingsCardTitleSkeleton \/>/)
  assert.match(source, /if \(!isLabelRevealed\) return <SettingsTabLabelSkeleton \/>/)
  assert.doesNotMatch(source, /if \(!cardViewModel\.config\.selectedType\) return fallbackGrid/)
})
