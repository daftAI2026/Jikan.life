...
test("md breakpoint routes main scrolling to outer main-content container", () => {
  ...
  assert.match(layoutTierSource, /const SIDEBAR_FORCE_MD_MAX = 1314/)
  assert.match(layoutTierSource, /if \(sidebarOpen && viewportWidth <= SIDEBAR_FORCE_MD_MAX\) return "md"/)
  ...
  assert.match(homePageSource, /const isEffectiveLg = effectiveLayoutTier === "lg"/)
  ...
  assert.match(homePageSource, /isEffectiveLg \? "md:overflow-y-hidden" : ""/)
  assert.match(homePageSource, /isEffectiveLg \? "md:h-full" : ""/)
  assert.match(homePageSource, /isEffectiveLg \? "md:h-full" : "",/)
  ...
  assert.match(homeGridSource, /const isEffectiveLg = effectiveLayoutTier === "lg"/)
  assert.match(homeGridSource, /isEffectiveLg \? "md:h-full md:grid-cols-2 md:divide-x md:divide-kumo-line md:overflow-hidden" : ""/)
  ...
  assert.match(settingsSource, /const isEffectiveLg = effectiveLayoutTier === "lg"/)
  assert.match(settingsSource, /isEffectiveLg \? "md:h-full md:overflow-y-hidden" : ""/)
})
...
test("Goal url card uses setup title and set flow guarded by copy success", () => {
  ...
  assert.match(gridSource, /const handleSetIt = async \(\) => \{[\s\S]*?const ok = await viewModel\.actions\.copyUrl\(\)/)
  assert.match(gridSource, /const handleSetIt = async \(\) => \{[\s\S]*?if \(!ok\) return/)
  ...
  assert.match(gridSource, /asideClassName="md:w-full md:border-l-0"/)
  assert.match(gridSource, /isEffectiveLg \? "md:hidden" : ""/)
  ...
  assert.match(paneSource, /const guideVisibilityClassName = effectiveLayoutTier === "md" \? "hidden" : "block"/)
  assert.match(paneSource, /visibilityClassName=\{guideVisibilityClassName\}/)
...
test("Setup guide panel uses local right-slide overlay with sidebar-aligned timing", () => {
  ...
  assert.match(source, /aria-modal=\{open\}/)
...
test("Registry settings cards only expose business ID selector", () => {
  ...
  assert.match(paneSource, /compactAtDesktop=\{isEffectiveLg\}/)
...
test("HomeGrid provides split workspace layout", () => {
  ...
  assert.match(source, /const isEffectiveLg = effectiveLayoutTier === "lg"/)
