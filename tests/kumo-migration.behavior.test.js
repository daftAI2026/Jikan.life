...
test("md breakpoint routes main scrolling to outer main-content container", () => {
  ...
  assert.match(homePageSource, /md:h-auto lg:h-full/)
  assert.match(homePageSource, /overflow-hidden border-r border-kumo-line md:h-auto lg:h-full/)
  ...
  assert.match(settingsSource, /lg:h-full/)
...
test("Goal url card uses setup title and set flow guarded by copy success", () => {
  ...
  assert.match(gridSource, /containerClassName="overflow-x-hidden"/)
  assert.match(gridSource, /asideClassName="md:w-full md:border-l-0"/)
  assert.match(gridSource, /className="pointer-events-none fixed top-\[49px\] right-\[49px\] bottom-0 left-12 z-40 hidden md:block lg:hidden"/)
  ...
  assert.match(paneSource, /visibilityClassName="md:hidden lg:block"/)
...
test("Setup guide panel uses local right-slide overlay with sidebar-aligned timing", () => {
  ...
  assert.match(source, /const DEFAULT_CONTAINER_CLASSNAME = "pointer-events-none absolute inset-0 z-40"/)
