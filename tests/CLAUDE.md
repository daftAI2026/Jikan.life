...
lock-screen-overlay-runtime.unit.test.js: 锁屏 overlay runtime helper 单测，锁定英文真实日期格式、Apple 判定、英文字体分流与午夜刷新计时。
...
变更日志
2026-03-11: 新增 `lock-screen-overlay-runtime.unit.test.js` 并更新 `kumo-migration.ui.behavior.test.js`：为锁屏 overlay 的真实日期、Apple/非 Apple 英文字体分流与午夜自动刷新补充运行时/源码双护栏，禁止 `date-text` 回退为写死 `Tue April 1`。
