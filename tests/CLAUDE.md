...
remotion-opening-timeline.unit.test.js: Remotion 开场动画时间线单测，锁定首行打字后停顿、闪烁输入光标、三列独立随机、三列 `慢->快->慢` 错拍节奏、英文排除与最终单行 `jikan.life` 尾声。
remotion-opening-layout.unit.test.js: Remotion 开场动画布局单测，锁定中心安全区不被左右文本侵入，且长拉丁串会触发字号回缩。
remotion-opening-fonts.unit.test.js: Remotion 开场动画字体单测，锁定首尾 `Inter` 锚点、中段共享风格组、中心安全池、高风险拉丁降级与拉丁样式变化。
