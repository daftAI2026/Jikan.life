...
remotion-opening-timeline.unit.test.js: Remotion 开场动画时间线单测，锁定首句单列打字后停顿、闪烁输入光标、三列独立随机、三列 `慢->快->慢` 错拍节奏、英文排除与最终单行 `jikan.life` 尾声。
remotion-opening-layout.unit.test.js: Remotion 开场动画布局单测，锁定固定 `125px` 中心安全区不被左右文本侵入，并要求长拉丁串继续通过字号回缩适配。
remotion-opening-fonts.unit.test.js: Remotion 开场动画字体单测，锁定首尾 `Inter` 锚点、中段共享风格组、中心列共享风格组池但仍保守字重/字形、`und` 排除 `IBM Plex Sans`、Han 字共享 CJK 随机池且可随机命中 `Zhi Mang Xing`、拉丁池包含 `Gloria Hallelujah`，以及高风险拉丁降级与拉丁样式变化。
opening-intro-overlay.behavior.test.js: 首页开场接入护栏，锁定 App 挂载点、Remotion 运行时直依赖、固定 `1080x1080` composition、lane 基于整张 stage 宽度定位、外层 `min(80vw, 80vh)` 缩放嵌入、禁止网页层覆盖 `fadeOut/debugLayout`，以及播放结束后自卸载语义。
...
变更日志
2026-03-20: 再次更新 `remotion-opening-fonts.unit.test.js`，为拉丁随机池增加 `Gloria Hallelujah` 存在性护栏，确保这款手写拉丁字不会被后续整理悄悄删掉。
2026-03-20: 再次更新 `remotion-opening-fonts.unit.test.js`，撤销 Han 的强制手写前置护栏，改为要求 Han 字回到共享 CJK 随机池，同时在随机样本里可命中 `Zhi Mang Xing`。
2026-03-20: 再次更新 `remotion-opening-fonts.unit.test.js`，将全列 Han 手写预览护栏从 `WDXL Lubrifont SC/TC` 替换为 `Zhi Mang Xing`，并同步调整“Han 不再属于旧共享 CJK 池”的断言语义。
2026-03-20: 再次更新 `remotion-opening-fonts.unit.test.js`，将 Han 手写预览护栏从“仅中心列”前移到“全列通用”，要求 left / center / right 的 Han 词都优先命中 `WDXL Lubrifont SC`。
2026-03-20: 再次更新 `remotion-opening-fonts.unit.test.js`，为中心 Han 词增加手写风格预览护栏：默认命中 `WDXL Lubrifont SC`，繁中 `與` 命中 `WDXL Lubrifont TC`。
2026-03-20: 再次更新 `remotion-opening-fonts.unit.test.js`，将中心列护栏从“独立保守池”改为“共享 `STYLE_GROUP_VARIANTS` 池但保留保守字重/字形”，同时继续要求 `und` 排除 `IBM Plex Sans`。
2026-03-20: 更新 `remotion-opening-fonts.unit.test.js`，为中心列拉丁词 `und` 增加排除 `IBM Plex Sans` 的护栏，防止当前 Studio 画面再次回到过宽过硬的中段字形。
2026-03-20: 更新 `remotion-opening-timeline.unit.test.js`，将开场整体速度护栏提升到 `1.5x`：intro/hold/hard cut/fade 全部改走缩短后的帧长，并把总时长断言从 `309f` 收敛到 `206f`。
2026-03-20: 更新 `remotion-opening-layout.unit.test.js`，将中心安全区护栏从“按中间文本动态伸缩”收敛为“默认固定 `125px` 盒宽，窄画布继续钳制”，与当前 Remotion Studio 调整语义保持同构。
...
2026-03-14: 新增 `remotion-opening-timeline.unit.test.js`，为独立 Remotion 开场动画锁定英文排除、seed 洗牌后的 11 条随机语言池、327 帧总时长与固定尾声 `時間 与 人生 -> 時間 と 人生 -> jikan と life -> jikan . life -> jikan.life` 语义。
2026-03-14: 新增 `remotion-opening-layout.unit.test.js`，将开场三列中心安全区与长文本字号回缩写成纯几何护栏，阻止左右文本再次跨中心重叠。
2026-03-14: 更新 `remotion-opening-timeline.unit.test.js`，新增 `Time & Life` 打字完成后 `12f` 停顿与中段三列独立随机组合护栏，总时长调整为 `339f`。
2026-03-14: 再次更新 `remotion-opening-timeline.unit.test.js`，将尾声改为 `309f` 总时长，并锁定 `9.0s -> 9.5s` 单行 `jikan.life` 后直接渐隐。
2026-03-14: 再次更新 `remotion-opening-timeline.unit.test.js`，新增左右两列 `慢->快->慢` 节奏护栏，并验证早段“中心先切、左列仍停留”的实际接线。
2026-03-14: 再次更新 `remotion-opening-timeline.unit.test.js`，把中心列也纳入错拍护栏，要求左/中/右三列都能在随机区里各自领拍一次。
2026-03-14: 新增 `remotion-opening-fonts.unit.test.js`，锁定首尾 `Inter` 锚点与中段按脚本分池的在线随机字体策略，并要求拉丁文本出现真实字重差异与斜体命中。
2026-03-14: 再次更新 `remotion-opening-fonts.unit.test.js`，要求随机场景共享同一风格组、中心列强制走保守池，并让带变音符的高风险拉丁串自动降级到安全字体。
2026-03-14: 再次更新 `remotion-opening-timeline.unit.test.js`，将打字机光标护栏从硬闪改为轻微明灭，要求光标只在 `intro-typewriter` / `intro-hold` 阶段以透明度呼吸，不会泄漏到随机段。
2026-03-20: 新增 `opening-intro-overlay.behavior.test.js`，锁定首页路由必须挂 `OpeningIntroOverlay`、`package.json` 必须显式声明 `@remotion/player`，并要求 overlay 在 Remotion 播放结束后自动淡出自卸载。
2026-03-20: 首页 overlay 曾改用浏览器 viewport 作为 composition 尺寸，并将整组动态文字外框固定到浏览器宽度的 `30%`，同时要求网页层显式覆盖 `fadeOut/debugLayout`。
2026-03-20: 再次更新 `opening-intro-overlay.behavior.test.js`，要求 Remotion Studio 默认 `OpeningTextAnimation` composition 改为 `1080x1080` 的 `1:1` 方形舞台。
2026-03-20: 再次更新 `opening-intro-overlay.behavior.test.js`，要求首页 overlay 回到固定 `1080x1080` composition，只通过外层 `min(80vw, 80vh)` 等比缩放嵌入，并禁止网页层继续改写 `fadeOut/debugLayout` 或 viewport 尺寸。
2026-03-20: 再次更新 `opening-intro-overlay.behavior.test.js`，要求 `OpeningTextComposition` 不再把三列限制在中间 `30%` 小容器，而是按整张 stage 宽度投射 left / center / right lane。
2026-03-20: 更新 `remotion-opening-timeline.unit.test.js`，要求 `intro-typewriter / intro-hold` 改为真正的单列 `lineText` 语义，防止首句 `Time & Life` 再被拆回 left / center / right 三列。
2026-03-20: 更新 `remotion-opening-layout.unit.test.js` 与 `remotion-opening-timeline.unit.test.js`，删除未接线 `compact` 死参数的伪断言，要求布局与尾声场景不再暴露这条假语义。
