...
test("MobileFooter provides mobile three-column footer with social links and centered language select", () => {
  ...
  assertNamedImports(source, "@/lib/utils", ["cn"])
  assert.match(source, /function MobileFooter\(\{ className, fixed = true \}\)/)
  assert.match(source, /const footerClassName = cn\(/)
  assert.match(source, /fixed \? "fixed inset-x-0 bottom-0 z-50" : null/)
  assert.match(source, /<div className="absolute inset-y-0 left-0 flex items-center px-3">\s*<LanguageSelect \/>/s)
  assert.match(source, /absolute inset-y-0 right-0 flex items-center px-3/)
  assert.match(source, /<div className="pointer-events-none absolute inset-0 grid place-items-center">[\s\S]*aria-label=\{mobileGithub\.label\}/)
...
test("HomeSidebar uses segmented mobile style selector and hides Life style card", () => {
  ...
  const cardStateSource = readSource("src/pages/registry/sections/home-sidebar-style-cards.js")
  ...
  assert.match(source, /const \[mobileViewingStyle, setMobileViewingStyle\] = useState\(\(\) => selectedStyle\)/)
  ...
  assert.match(source, /useEffect\(\(\) => \{\s*setMobileViewingStyle\(selectedStyle\)\s*\}, \[selectedStyle\]\)/)
  ...
  assert.match(cardStateSource, /function resolveVisibleStyleCards\(styleCards\)/)
  assert.match(cardStateSource, /function resolveSidebarActiveStyleId\(selectedStyle, visibleStyleCards\)/)
  assert.match(cardStateSource, /return visibleStyleCards\[0\]\?\.id \?\? null/)
  ...
  assertNamedImports(cardsSource, "@/components/ui/kumo", ["Tabs"])
  assertNamedImports(cardsSource, "./MobileFooter", ["MobileFooter"])
  assertNamedImports(cardsSource, "./home-sidebar-style-cards", ["resolveSidebarActiveStyleId", "resolveVisibleStyleCards"])
  assert.match(cardsSource, /layoutMode = "desktop-list"/)
  assert.match(cardsSource, /if \(layoutMode === "mobile-segmented"\)/)
  assert.match(cardsSource, /variant="segmented"/)
  assert.match(cardsSource, /className: "min-w-0 basis-0 flex-1 justify-center"/)
  assert.match(cardsSource, /value=\{activeStyleId\}/)
  assert.match(cardsSource, /onValueChange=\{onViewedStyleChange\}/)
  assert.match(cardsSource, /className="flex min-h-0 flex-1 flex-col overflow-hidden"/)
  assert.match(cardsSource, /isMobileSegmented \? "h-full min-h-0 flex-1"/)
  assert.match(cardsSource, /isMobileSegmented \? "min-h-0 flex-1"/)
  assert.match(cardsSource, /className="-mx-3 flex items-center overflow-hidden bg-kumo-elevated"/)
  assert.match(cardsSource, /style=\{\{ height: "calc\(var\(--registry-topbar-height\) \+ 0\.75rem\)" \}\}/)
  assert.match(cardsSource, /className="flex h-\[var\(--registry-topbar-height\)\] w-full items-center px-3"/)
  assert.match(cardsSource, /<div className="w-full px-1\.5">\s*<Tabs/s)
  assert.match(cardsSource, /<MobileFooter fixed=\{false\} className="-mx-3" \/>/)
  assert.match(source, /layoutMode="desktop-list"/)
  assert.match(source, /layoutMode="mobile-segmented"/)
  assert.match(source, /viewedStyle=\{mobileViewingStyle\}/)
  assert.match(source, /onViewedStyleChange=\{setMobileViewingStyle\}/)
  assert.match(source, /className="min-h-0 grow overflow-hidden px-3 pt-3 pb-0 text-sm"/)
  ...
  assertNamedImports(cardsSource, "./home-sidebar-mixed-text", ["splitSidebarMixedTextRuns"])
...
test("HomeSidebar delegates style-card rendering to HomeSidebarCards with callback bridge", () => {
  ...
  assert.match(cardsSource, /function HomeSidebarCards\(\{/)
  assert.match(cardsSource, /layoutMode = "desktop-list"/)
  assert.match(cardsSource, /selectedStyle,/)
  assert.match(cardsSource, /viewedStyle,/)
  assert.match(cardsSource, /onStyleSelect,/)
  assert.match(cardsSource, /onViewedStyleChange,/)
  assert.match(cardsSource, /yearStats,/)
  ...
  assert.match(cardsSource, /title:\s*renderMixedSidebarText\(t\("type\.year\.name",\s*\{\s*year:\s*String\(yearStats\.year\)\s*\}\),\s*lang\)/)
  assert.doesNotMatch(cardsSource, /titleTrailing:\s*String\(yearStats\.year\)/)
  assert.doesNotMatch(cardsSource, /style\.titleTrailing/)
  assert.match(cardsSource, /inlineDay:\s*renderMixedSidebarText\(t\("type\.year\.inlineDay",\s*\{\s*n:\s*String\(yearStats\.day\)\s*\}\),\s*lang\)/)
  assert.match(cardsSource, /inlineComplete:\s*renderMixedSidebarText\(t\("type\.year\.inlineComplete",\s*\{\s*n:\s*`\$\{yearStats\.percent\}%`\s*\}\),\s*lang\)/)
  assert.match(cardsSource, /const JAPANESE_MIXED_TEXT_FONT_FAMILY = '"Noto Sans JP", "Inter", ui-sans-serif, system-ui, sans-serif'/)
  assert.match(cardsSource, /splitSidebarMixedTextRuns\(text,\s*lang\)/)
  assert.match(cardsSource, /run\.kind === "ja" \? \{ fontFamily: JAPANESE_MIXED_TEXT_FONT_FAMILY \} : undefined/)
})
