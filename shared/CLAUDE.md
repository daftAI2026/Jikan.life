...
random-palette.js: 随机配色生成器，复用 WCAG 对比度规则产出可读的背景/强调色对，供 workspace 的随机预设入口消费。
...
**消费方**：
- `src/pages/registry/sections/workspace/{HomePreviewPane,YearPreviewSvg,GoalPreviewSvg}.jsx`（Browser inline SVG 预览）
- `worker/generators/*.js`（SVG 适配）

**不可包含**：任何浏览器/Node 特定 API。必须保持纯函数。

变更日志
2026-03-27: 新增 `random-palette.js` 作为共享随机配色真相源；`palettes.js` 的 `preset-8` 改为显式 `kind="random"` 元数据，供 workspace 把第 8 个 preset 收口为随机入口而非静态色对。
