...
成员清单
LockScreenOverlay.jsx: overlay 渲染器，固定 `402x874` 坐标系；Widgets/Status 继续 inline，底部 controls 改为 `shadow svg + glass dom + chrome svg` 混合分层，glass DOM 现运行在和 SVG 同构的 `402x874` 绝对平面并通过 `overlayScale` 统一缩放，主时钟改为 `120/Bold` 的居中文本并输出真实 24 小时制时间，左上角时间同步改成真实时间，除 `date-text` 外其余文本继续复用既有英文 overlay 字体策略，`date-text` 单独改为真实本地日期，并支持上游通过 `showWidgets` 整组隐藏四个 circular widgets
...
变更日志
2026-03-12: `LockScreenOverlay.jsx` 新增 `showWidgets` 可见性入口，并以单一条件包裹 `translate(30 679)` 整组 widgets；Goal preview 现可整组隐藏四个 circular widgets，而不改 layer id、颜色协议或其他 overlay 元素。
