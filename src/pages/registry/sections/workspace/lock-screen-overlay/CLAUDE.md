...
成员清单
LockScreenOverlay.jsx: overlay 渲染器，固定 `402x874` 坐标系；Widgets/Status 继续 inline，底部 controls 改为 `shadow svg + glass dom + chrome svg` 混合分层，主时钟改为 `120/Bold` 的居中文本并输出真实 24 小时制时间，左上角时间同步改成真实时间，普通文本统一复用 overlay 文本字体策略，`date-text` 保持真实英文日期
