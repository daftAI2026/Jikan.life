...
lock-screen-overlay.colors.js: overlay 私有配色映射层，把 workspace accentColor 投影到主时钟/日期/widgets，并维持 widgets `fg = accent / bg = accent 15% alpha` 关系，明确排除 top 层时间
...
法则: 坐标固定·layer id 稳定·颜色入口集中·日期/时间 runtime 化但几何不漂移·accent 驱动主时钟/日期/widgets 且 top 状态栏独立·Sketch live 层级优先·Stack 静态化例外·禁止回退整图黑盒
