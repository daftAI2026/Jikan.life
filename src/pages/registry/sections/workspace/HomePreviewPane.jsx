/**
 * [INPUT]: 依赖 react hooks, @/lib/renderer(drawYearProgress/drawLifeCalendar/drawGoalCountdown), LockScreenPreviewFrame
 * [OUTPUT]: 对外提供 HomePreviewPane 组件（Figma 锁屏壳 + 空态提示文案/实时 Canvas 预览）
 * [POS]: registry/sections/workspace 的左侧预览面板，根据选型状态切换提示文案与真实壁纸渲染，并按导出坐标严格等比缩放后投影到固定锁屏 Wallpaper 槽位
...
function HomePreviewPane({ config, selectedDevice, t }) {
    ...
    return (
        <div className="flex h-full min-h-[420px] flex-col items-center justify-center gap-4 px-6 py-8">
            <LockScreenPreviewFrame showOverlay={Boolean(config.selectedType)}>
                {config.selectedType ? (
                    <canvas
                        ref={canvasRef}
