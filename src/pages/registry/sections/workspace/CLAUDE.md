...
变更日志
2026-03-11: `lock-screen-overlay/LockScreenDarkOverlay.jsx` 的 `date-text` 改为运行时真实英文日期，并从短文案左起点坐标收口为锁屏中线居中锚点；新增 `lock-screen-overlay/lock-screen-overlay.runtime.js` 收口英文日期格式、Apple/非 Apple 字体分流（Apple=系统字体，非 Apple=`Inter`）与跨午夜自动刷新计时，字号/字重保持不变。
