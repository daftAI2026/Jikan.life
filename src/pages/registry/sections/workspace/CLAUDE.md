...
HomeSettingsPane.jsx: 右侧设置面板主容器；回归 pane 编排层，只负责卡片顺序、6 站位空态 Skeleton Base、`revealStage` 渐进解锁、`md + drawer open` 下的 bottom-tabs JSX、title/tab/body skeleton 与 `useAnchoredSetupRow` 语义收口；不再直接持有 tabs 测量/resize/indicator 状态，Guide 宿主继续读取 `shouldRenderPaneGuideHost`，导出 `SETTINGS_CARD_IDS`
...
结构
workspace/ - Home 双栏工作区子模块 (13 files + cards/ 子目录)
