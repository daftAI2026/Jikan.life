...
colors-card.jsx: Colors 业务卡，实现 Background/Accent 双 ColorPicker、Accent 字段 tooltip 与 presets 应用动作；`preset-8` 改为黑色 token 的 `Shuffle` 随机入口（effective `mid` 时 year/goal 隐藏 presets）。
