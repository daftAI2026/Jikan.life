...
RegistrySidebar.jsx: 本地受控侧栏，负责样式卡片选择与侧栏开合交互（文案走 i18n）
ThemeToggle.jsx: vendor Kumo ThemeToggle 的薄包装（同源挂载）
LanguageSelect.jsx: Registry UI 语言切换组件（触发器为“线框地球图标+语言名”，菜单为“国旗+语言名”且保留默认选中勾，二者间距统一；桌面顶栏左侧挂载，移动端保留右下角入口）
SearchDialog.jsx: vendor Kumo SearchDialog 的薄包装（同源挂载）
KumoMenuIcon.jsx: vendor Kumo KumoMenuIcon 的薄包装（同源挂载）
