...
成员清单
HomeTopbar.jsx: 顶栏区域（桌面），支持 hideLanguage（侧栏展开时隐藏左侧语言切换），右侧展示 GitHub 与小红书社交入口
HomeSidebar.jsx: 本地侧栏，负责样式卡片选择与侧栏开合交互（支持 sidebarOpen/onSidebarOpenChange 受控接口，文案走 i18n），GoalVisual 圆环粗细 strokeWidth=2.5
...
KumoMenuIcon.jsx: Home 本地菜单动效图标（避免跨包引用 vendor docs 源码）
