...
- **Edge Routing Strategy**: Expanded `wrangler.toml` and Worker route dispatching to prioritize `/` and `/og-image.png` at the edge layer. This ensures that the root `index.html` is always processed by the Worker before delivery, enabling real-time metadata injection while maintaining high-performance asset delivery.
...
- **Dynamic Origin Resolution**: Upgraded the Worker edge layer to programmatically resolve the OG image URL based on the incoming `url.origin`. This ensures that staging and preview deployments correctly point to their respective environment-specific OG endpoints rather than hardcoded production URLs.
...
- **Year Preview Visual Consistency**: Refined the dot rendering logic in `YearPreviewSvg` to only apply reduced fill opacity (`0.75`) to completed days that are *not* today. This ensures the current day highlight remains at full opacity even when marked as completed, improving visual clarity of the active date.
...
### Infrastructure
- **Dependency Upgrades**: Synchronized packages to their latest versions, including `@cloudflare/kumo` to `1.15.0` and `wrangler` to `4.76.0`.
